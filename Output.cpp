...
Output::Output()
{
	...
	UI.PlayerColors[0] = BLUE;
	UI.PlayerColors[1] = RED;
	UI.PlayerColors[2] = GREEN;
	UI.PlayerColors[3] = ORANGE;
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_GAME_OBJECT] = "images\\DELETE_GAME.jpg";
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (toCell.HCell() == fromCell.HCell() && fromCell.VCell() > toCell.VCell());
	else
		return;
	...
	pWind->DrawLine(x34, y1, x34+1, y2+1);
	...
	for (int i = 0; i < (fromCell.VCell() - toCell.VCell()); i++) {
		pWind->DrawLine(x12, z, x34 + 1, z + 1);
		z = z - UI.CellHeight;
	}
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	int y_up = y1 + yChange;
	int y_down = y1 - yChange;
	int x_right = x12 + xChange;
	int x_left = x12 - xChange;

	int px[5] = { x_left,x12,x_right,x12,x_left };
	int py[5] = { y1,y_up,y1,y_down,y1 };
	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way

	pWind->DrawPolygon(px, py, 5);
