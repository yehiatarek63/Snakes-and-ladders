...
{	
	...
	else	
	{
		...
		case ITM_EXIT_: return EXIT;
		case ITM_SWITCH_TO_DESIGN_MODE: return TO_DESIGN_MODE_;
...
CellPosition Input::GetCellClicked() const
{
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		if ( y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			...
			float y_coordinate = ((y - UI.ToolBarHeight)/UI.CellHeight);
			float x_coordinate = (x/UI.CellWidth);
			cellPos.SetHCell(int(x_coordinate));
			cellPos.SetVCell(int(y_coordinate));
		}
	}
