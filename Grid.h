...
class Grid
{
	...
	static Card * Clipboard;	   // This is used in copy/cut/paste card (should be set in copy/cut and got in paste)
...
public:
	...
	bool HasLadderSnake(CellPosition pos);
	void ClearGrid();
	...
	int GetCountofObj(int type) const;
	Player * GetCurrentPlayer() const;
	Player* GetNextPlayer() const;	// Gets a Pointer to the Current Player	                                    
	...
	GameObject** GetGameObjectList() const;
	bool IsOverlapping(GameObject* newObj);
	Player* GetPlayerByNum(int x);
	void GetLightingPlayerExpect(int x);
	void ResetAll9to11();
	...
	void SaveAll(ofstream& OutFile, Grid* pGrid, int typ);

	void LoadAll(ifstream& InFile, Grid* pGrid, int typ);
