...
CellPosition CellPosition::GetCellPositionFromNum(int cellNum)
{
	...
	if (cellNum % 11 == 0) {
		position.SetHCell((cellNum % 11) + 10);
		position.SetVCell(ceil(abs((cellNum / 11) - 9)));
	}
	else if (cellNum <= 99 && cellNum >= 1) {
...
void CellPosition::AddCellNum(int addedNum)
{
	...
	if ((GetCellNumFromPosition(*this) + addedNum) <= 99) {
		CellPosition cellpos = GetCellPositionFromNum(addedNum + GetCellNumFromPosition(*this));
		vCell = cellpos.VCell();
		hCell = cellpos.HCell();
	}
	else {
